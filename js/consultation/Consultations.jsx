...
export default class Consultations extends React.Component {
    ...
    onNewObject = (object, sender) => {
        switch (object.type) {
            ...
            case "filter":
                const photos = Object.assign({}, this.state.photos);
                photos[object.photoId].filters = object.filters;
                this.setState({photos});
        }
    };
    ...
    applyFilter = (filter, photoid) => {
        //const newFilter = this.state.photos[photoid].filters.concat(filter);
        const photos = Object.assign({},this.state.photos);
        photos[photoid].filters = this.state.photos[photoid].filters.concat(filter);
        this.setState({photos});

        const object = {photoId : photoid, filters: photos[photoid].filters, type: "filter"};
